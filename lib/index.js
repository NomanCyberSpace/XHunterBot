...
async function addWelcome(jid, enabled, message) {
    try {
        ...
        data.welcome[jid] = {
            ...
            channelId: '120363321458823123@newsletter'
        };
...
async function setGroupOnlyAdmins(groupId, enabled) {
    try {
        const data = loadUserGroupData();
        if (!data.onlyAdmins) data.onlyAdmins = {};
        data.onlyAdmins[groupId] = enabled;
        saveUserGroupData(data);
        return true;
    } catch (error) {
        console.error('Error setting group only admins:', error);
        return false;
    }
}

async function getGroupOnlyAdmins(groupId) {
    try {
        const data = loadUserGroupData();
        return data.onlyAdmins?.[groupId] || false;
    } catch (error) {
        console.error('Error getting group only admins:', error);
        return false;
    }
}
