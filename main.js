...
async function handleMessages(sock, messageUpdate, printLog) {
    try {
        ...
        if (message.message?.buttonsResponseMessage) {
            ...
            if (buttonId === 'channel') {
                await sock.sendMessage(chatId, { 
                    text: '📢 *Join our Channel:*\nhttps://whatsapp.com/channel/0029Vb8IFMaLY6cxSYppll2g' 
                }, { quoted: message });
                return;
                ...
                await sock.sendMessage(chatId, { 
                    text: `🔗 *Support*\n\nhttps://chat.whatsapp.com/FeA7ijiTOpUCKNKjMlYasG` 
                }, { quoted: message });
                return;
            }
        }
        ...
        if (!userMessage.startsWith('.')) {
            ...
            if (isGroup) {
                // Delete messages mentioning "status"
                if (userMessage.includes('status')) {
                    const adminStatus = await isAdmin(sock, chatId, senderId);
                    if (adminStatus.isBotAdmin && !adminStatus.isSenderAdmin && !isOwnerOrSudoCheck) {
                        await sock.sendMessage(chatId, { delete: message.key });
                        return;
                    }
                }

                // Check if group is in "only admins" mode
                const onlyAdmins = await getGroupOnlyAdmins(chatId);
                if (onlyAdmins) {
                    const adminStatus = await isAdmin(sock, chatId, senderId);
                    if (adminStatus.isBotAdmin && !adminStatus.isSenderAdmin && !isOwnerOrSudoCheck) {
                        await sock.sendMessage(chatId, { delete: message.key });
        ...
        switch (true) {
            ...
            case userMessage === '.on':
                if (isGroup) {
                    ...
                    await sock.sendMessage(chatId, { text: '🛡️ Group permission: *ON* (Only admins can send messages)', ...channelInfo }, { quoted: message });
                }
                break;
            case userMessage === '.off':
                if (isGroup) {
                    ...
                    await sock.sendMessage(chatId, { text: '🛡️ Group permission: *OFF* (Everyone can send messages)', ...channelInfo }, { quoted: message });
                }
                break;
            case userMessage.startsWith('.pair'):
                {
                    const phoneNumber = userMessage.split(' ')[1];
                    if (!phoneNumber) {
                        await sock.sendMessage(chatId, { text: 'Usage: .pair <number>\nExample: .pair 923001234567', ...channelInfo }, { quoted: message });
                        break;
                    }
                    await sock.sendMessage(chatId, { text: '🛡️ *XHUNTERBOT PAIRING*\n\nGenerating pairing code for: ' + phoneNumber + '\n\nPlease wait about 10-15 seconds. This will NOT logout your main bot.', ...channelInfo }, { quoted: message });
                    ...
                    try {
                        ...
                        // Use a unique subfolder in a dedicated temp directory
                        const tempRoot = path.join(process.cwd(), 'temp_pairings');
                        if (!existsSync(tempRoot)) mkdirSync(tempRoot, { recursive: true });
                        const tempSessionPath = path.join(tempRoot, `session_${Date.now()}`);
                        if (!existsSync(tempSessionPath)) mkdirSync(tempSessionPath, { recursive: true });
                        ...
                        const tempSock = makeWASocket({
                            ...
                            // Use macOS Chrome browser as recommended for pairing stability
                        ...
                        // IMPORTANT: Wait for the socket to be fully ready to send requests
                        // According to community research, waiting for 'connecting' or a small delay helps avoid "Connection Closed"
                        ...
                        await sock.sendMessage(chatId, { 
                            text: `🛡️ *XHUNTERBOT PAIRING CODE*\n\nNumber: ${cleanNumber}\nCode: *${formattedCode}*\n\nUse this code to pair your second number. Your main bot is safe and connected.`,
                            ...channelInfo 
                        }, { quoted: message });
                        
                        // Cleanup after 3 minutes to allow user to use the code
                        setTimeout(() => {
                            try {
                                tempSock.logout();
                                tempSock.end();
                                setTimeout(() => rmSync(tempSessionPath, { recursive: true, force: true }), 5000);
                            } catch (e) {}
                        }, 180000);

                    } catch (e) {
                        console.error('Pairing error:', e);
                        await sock.sendMessage(chatId, { text: '❌ *Pairing Failed*\n\nError: ' + (e.message || 'Connection Closed') + '\n\nPossible reasons:\n1. Rate limit (too many requests)\n2. Internet connection issue\n3. WhatsApp server busy\n\nPlease try again in a few minutes.', ...channelInfo }, { quoted: message });
                    }
                }
                break;
